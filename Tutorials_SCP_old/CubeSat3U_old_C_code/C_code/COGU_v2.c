...
double omega_max_double = 10.0 * M_PI / 180.0;
...
/* RK5 step (explicit) used in Python implementation */
void rk5_step(double xk[STATE], double uk[CTL], double dt, double xkp1[STATE])
{
    double k1[STATE], k2[STATE], k3[STATE], k4[STATE], k5[STATE], k6[STATE];
    double tmp[STATE];

    f_qw(xk, uk, k1);

    for (int i=0;i<STATE;i++)
        tmp[i] = xk[i] + 0.25 * dt * k1[i];
    f_qw(tmp, uk, k2);
...
void COGU_guidance(double u_global[CTL][T], double x_global[STATE][TP1], double start_quat[4], double end_quat[4]) {
    ...
    while (iter <= KMAX) {
        ...
        for (int k = 0; k < T; k++) {
            ...
            double hpow = tau;
            double fact = 1.0;
            for (int ord = 2; ord <= TAYLOR_ORDER + 1; ord++) {
