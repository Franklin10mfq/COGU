...
from COGU_algorithms.Waskiri_Sat_official.BSimp.WaskiriSat_T51_Bsimp_QOCO import cpg_module
...
def cpg_solve(prob, updated_params=None, **kwargs):
    ...
    prob.var_dict['u'].save_value(np.array(res.cpg_prim.u).reshape((3, 51), order='F'))
    prob.var_dict['vc'].save_value(np.array(res.cpg_prim.vc).reshape((7, 50), order='F'))
    prob.var_dict['x'].save_value(np.array(res.cpg_prim.x).reshape((7, 51), order='F'))
    ...
    results_dict = {'solver_specific_stats': solver_specific_stats,
                    ...
                    'solve_time': t1 - t0}
    prob._solver_stats = SolverStats.from_dict(results_dict, 'QOCOgen')
